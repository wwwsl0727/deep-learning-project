...
class EncoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        Layers: 
        input ---> H_in(H), W_in(W) 
        conv1 ---> kernel size: (3,3),S=1 ,P=3-1/2=1, D=1  n_filters:64 (same padding)
        relu1
        conv2 ---> kernel size: (4,4),S=2 ,P=1, D=1  n_filters:64 -> H/2,W/2
        relu2 
        conv3 ---> kernel size: (3,3),S=1 ,P=1, D=1  n_filters:128
        relu3
        conv4 ---> kernel size: (4,4),S=2 ,P=1, D=1  n_filters:128 -> H/4, W/4
        relu4
        conv5 ---> kernel size: (3,3),S=1 ,P=1, D=1  n_filters:256
        relu5
        conv6 ---> kernel size: (4,4),S=2 ,P=1, D=1  n_filters:256 -> H/8, W/8
        relu6
        conv7 ---> kernel size: (3,3),S=1 ,P=1, D=1  n_filters:512
        relu7
        #FC1 --> H/8*W/8*512
        """

        self.filters = [64,64,128,128,256,256]
        in_filters = self.filters.copy()
        in_filters.insert(0, in_channels)
        out_filters = self.filters.copy()
        out_filters.insert(len(self.filters),out_channels)
        self.pad_sz = 1
        self.kernel_odd_sz = 3
        self.stride_odd_sz = 1
        self.kernel_even_sz = 4
        self.stride_even_sz = 2
        # H_in = 64
        # W_in = 64
        # H_out = H_in
        # W_out = W_in
        ...
        for i, (in_dim, out_dim) in enumerate(zip(in_filters, out_filters)):
            if i%2 ==0:
            #For odd layer
                modules.append(spectral_norm(
                    nn.Conv2d(in_dim, out_dim, kernel_size = self.kernel_odd_sz, padding=1, stride=self.stride_odd_sz)))
                modules.append(nn.LeakyReLU(negative_slope=0.1))
            else:
            #For even layer
                modules.append(spectral_norm(
                    nn.Conv2d(in_dim, out_dim, kernel_size=self.kernel_even_sz, padding=1, stride=self.stride_even_sz)))
                modules.append(nn.LeakyReLU(negative_slope=0.1))
        ...
        for i, (in_dim, out_dim) in enumerate(zip(in_filters, out_filters)):
            if i!=3:
                modules.append(nn.ConvTranspose2d(
                    in_dim, out_dim, kernel_size=self.kernel1_sz, padding=self.pad_sz, stride=self.stride1_sz))
                modules.append(nn.BatchNorm2d(out_dim))
                modules.append(nn.ReLU())
            else:
                modules.append(nn.ConvTranspose2d(
                    in_dim, out_dim, kernel_size=self.kernel2_sz, padding=self.pad_sz, stride=self.stride2_sz))
        # ========================
        self.cnn = nn.Sequential(*modules)
